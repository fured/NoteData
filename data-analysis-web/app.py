...
#		pdb.set_trace()
		...
		if request.path.startswith("/blog/") and len(request.path) > 20:
			request.__blogid__ = request.path[6:]
		if request.path.startswith("/api/blogs/") and len(request.path) > 20:
			request.__blogid__ = re.findall("/api/blogs/(.*?)/comments",request.path)
			print(request.__blogid__)
...
def init(loop):
	print("1")
	yield from oorm.create_pool(loop,user = "user-test",password = "test-password",db = "testdb")
